...
Установка и поддержка скрипта: @artemproger
...
🔗 Ваша ссылка для приглашений: https://t.me/X2ROBOT?start=${message.from.id}
...
🌐 Отправьте любую сумму на кошелек <code>+79644455788</code>
...
`, {
				...
				reply_markup: {
					inline_keyboard: [
                        [{ text: "💴 Выводы", url: "https://t.me/Otziviprojekt" }],					
					]
				}
			});
		}
...
🌐 Отправьте любую сумму на кошелек <code>+79644468932</code>
‼️ с комментарием <code>BP${message.chat.id}</code>`, {
			parse_mode: "HTML"
		});
	}
...
▫️ @Brawll_Starschat`, {
	...
	if (query.data == 'game_prize_check') {
		var res = await bot.getChatMember("@AnimeInvest_pay", message.chat.id)
		if (message.user.prize) return bot.answerCallbackQuery(query.id, '🙂 Вы уже получили свой подарок!', true);
	...
	if (query.data.startsWith('withdraw:')) {
		...
		bot.sendMessage("@Pro2xinvest_pay", `<a href="tg://user?id=${ticket.id}">Пользователь</a> вывел <b>${ticket.amount}₽</b>\nПС: QIWI`, { parse_mode: "HTML" })
	
		if (ticket.wallet.indexOf("P") == -1) { // Платёж через QIWI
		  qiwi.toWallet({ account: String(ticket.wallet), amount: ticket.amount, comment: 'Выплата от @X2MONEYROBOT' }, () => { });
		}
	...
	`, {
		  parse_mode: "html", reply_markup: {
			inline_keyboard: [
			  [{ text: "👨‍💻 Админ", url: "https://t.me/AnimeIn" },
			  { text: "💬 Чат", url: "https://t.me/AnimeInv" }],
...
`, {
			...
			bot.sendMessage(uid, `Чек создан:\nhttps://t.me/AnimeInvest_bot?start=C${cid}`, { reply_markup: RM_admin_return })
		}
  ...
  /*      if (body.history[txnId].comment.startsWith('ZG')) {
        ...
        if (b == 0) {
          ...
          bot.sendMessage(id, `Ваш баланс пополнен на ${sum}₽`);
		      bot.sendMessage("@Pro2xinvest_pay", `<a href="tg://user?id=${id}">Пользователь</a> пополнил <b>${sum}₽</b>\nПС: PAYEER`, { parse_mode: "HTML" })
          ...
          bot.sendMessage(id, `Ваш баланс пополнен на ${sum}₽ и Вы получаете бонус - ${roundPlus(sum * b)}₽!`);
		      bot.sendMessage("@Pro2xinvest_pay", `<a href="tg://user?id=${id}">Пользователь</a> пополнил <b>${sum}₽ и получил бонус - ${roundPlus(sum * b)}p!</b>\nПС: PAYEER`, { parse_mode: "HTML" })
	...
	qiwi.getOperationHistory({ rows: 10, operation: 'IN' }, (err, response) => {
		response.data.map(async (x) => {
			...
			if (b > 0) {
				...
				bot.sendMessage("@Pro2xinvest_pay", `⚡️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b> и получил ${roundPlus(x.sum.amount * b)}₽ бонусом!\nПС: QIWI`, { parse_mode: "HTML" })
				...
				bot.sendMessage("@Pro2xinvest_pay", `⚡️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b>\nПС: QIWI`, { parse_mode: "HTML" })
